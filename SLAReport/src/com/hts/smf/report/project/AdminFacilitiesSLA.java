...
public class AdminFacilitiesSLA implements ReportConstants {
	...
	private void calucalateSLA(List<IssuesBean> issuesList) throws ParseException {			
		...
		if((issueRejectedDate.get(Calendar.YEAR) == 1900) &&(issueRejectedDate.get(Calendar.MONTH) == 01)) {
			...
			if(assignIndex > 0 && wipIndex > 0 && resolvedIndex > 0) {
				...
				setResultsToBean(issueKey,priority, serviceName, businessUnit,location, reOpenReason, changeStatus, issueOpenDateandTime, isAssignedDate, isResolvedDate,
				...
				setResultsToBean(issueKey,priority, serviceName, businessUnit, location, reOpenReason, changeStatus, issueOpenDateandTime, isAssignedDate, "",
						responseTimeInMinutes, resolvedTimeInMinutes, onHoldTimeinMinutes,
						responseSLAMsg, OPEN);
			...
			setResultsToBean(issueKey,priority, serviceName, businessUnit, location, changeStatus, reOpenReason, issueOpenDateandTime, REJECTED+":"+isRejectedDate, REJECTED,
					responseTimeInMinutes, resolvedTimeInMinutes, onHoldTimeinMinutes,
					REJECTED, REJECTED);
			}
		}		
	...
	private String resolvedSLA(long resolvedTime, String propPriority) {				
		...
		if(propPriority.equals("Cat3")){
			if((wLocation.equals("BangaloreAdmin")) || (wLocation.equals("BangaloreFacilities"))||
					(wLocation.equals("MaduraiOperations")) || (wLocation.equals("HyderabadOperations")) || (wLocation.equals("Transport"))){
				return "";
			}
		} else if(propPriority.equals("Cat2") && wLocation.equals("BangaloreFacilities")){
			return "";
		} else if(propPriority.equals("Cat2")){
			if(wLocation.equals("HyderabadOperations") || wLocation.equals("MaduraiOperations")){
				if((wTicketType.equals("Facilities"))){
					return "";
				}
			}
		}			
