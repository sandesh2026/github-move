...
public class HSESLA implements ReportConstants {
	...
	private void calucalateSLA(List<IssuesBean> issuesList) throws ParseException {			
		...
		if((issueRejectedDate.get(Calendar.YEAR) == 1900) &&(issueRejectedDate.get(Calendar.MONTH) == 01)) {
			...
			if((assignIndex > 0 && wipIndex > 0 && resolvedIndex > 0) || (resolvedIndex > 0 && wRInputs.getTicketType().replace(" ", "").equals(WOKK_PERMIT))) {
				...
				setResultsToBean(issueKey,priority, serviceName, businessUnit,location, changeStatus, issueOpenDateandTime, isAssignedDate, isResolvedDate,
					...
					String responseSLAMsg = resposeSLA(responseTimeInMinutes);				
				setResultsToBean(issueKey,priority, serviceName, businessUnit, location, changeStatus, issueOpenDateandTime, isAssignedDate, "",
						responseTimeInMinutes, resolvedTimeInMinutes, onHoldTimeinMinutes,
						responseSLAMsg, OPEN);
			...
			if(issueRejectedDate.get(Calendar.YEAR) != 1900){			
				...
				setResultsToBean(issueKey,priority, serviceName, businessUnit, location, changeStatus, issueOpenDateandTime, REJECTED+":"+isRejectedDate, REJECTED,
						responseTimeInMinutes, resolvedTimeInMinutes, onHoldTimeinMinutes,
						REJECTED, REJECTED);
			}
		}		
	...
	private void setResultsToBean(String issueName, String priorityName,
			String serviceNames, String businessUnit, String location, String issueStatus, String openDate, String assignDate,
